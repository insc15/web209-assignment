...
function Button({ as: Tag = 'button', children, className = '', onClick = () => {return;}, isLoading = false, ...rest }: ButtonProps) {
  ...
  return <Element disabled={isLoading} {...rest} onClick={onClick} className={`bg-primary rounded disabled:bg-gray-400 text-white disabled: hover:bg-button-hover duration-150 flex items-center justify-center ${className} ${className.includes('px') ? '' : 'px-3'} ${className.includes('py') ? '' : 'py-2'} }`}>
    {children}
